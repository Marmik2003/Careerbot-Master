...
class ChatInterface(Frame):
    ...
    def __init__(self, master=None):
        ...
        self.text_box = Text(self.text_frame, yscrollcommand=self.text_box_scrollbar.set, state=DISABLED,
                             bd=1, padx=6, pady=0, spacing3=0, wrap=WORD, bg="#4f4f4f",fg = "#ffffff", font=("Verdana 10", 8), relief=GROOVE,
                             width=10, height=1)
        self.text_box.pack(expand=True, fill=BOTH)
        self.text_box_scrollbar.config(command=self.text_box.yview)
        
        self.text_box.configure(state=NORMAL)
        self.text_box.insert(END, "Welcome to CareerBot v1.0.2 \nI can help you to decide your Career. \nFor more info you can click on help! \nYou Have these Options: \n      10th Pass \n      12th Pass \n")
        ...
        self.send_button = Button(self.send_button_frame, text="Send", width=5, relief=GROOVE,
                                  bd=1, command=lambda: self.send_message_insert(None),bg = "#4f4f4f", fg = "#ffffff", activebackground="#4f4f4f",
    ...
    def send_message_insert(self, message):
        ...
        repstr = botmind.chat_reply(botmind.word_dict, ui_lower)
